<?php

declare(strict_types=1);

namespace ApplicationManagerTools\AmDriver\Bridge\Symfony\OrchestrationCommand\Presenters;

use ApplicationManagerTools\AmDriver\Application\Service\OrchestrationCommand\ProcessOrchestrationCommand\ProcessOrchestrationCommandServiceResponse;
use ApplicationManagerTools\AmDriver\Application\Service\Shared\PresenterInterface;
use ApplicationManagerTools\AmDriver\Application\Service\Shared\Response;
use stdClass;

class PresenterProcessOrchestrationCommand implements PresenterInterface
{
    /** @var ProcessOrchestrationCommandServiceResponse */
    private $response;

    public function write(Response $response): void
    {
        if ($response instanceof ProcessOrchestrationCommandServiceResponse) {
            $this->response = $response;
        }
    }

    public function read(): stdClass
    {
        $result = new stdClass();
        $result->accepted = true;
        $result->alreadyProcessed = $this->response->alreadyProcessed;

        return $result;
    }
}
